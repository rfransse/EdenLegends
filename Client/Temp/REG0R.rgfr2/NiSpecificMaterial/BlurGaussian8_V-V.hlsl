#line 2 "D:\EdenClients\DepotContainer\EdenWorking-\Client\Temp\REG0R.rgfr2\NiSpecificMaterial\BlurGaussian8_V-V.hlsl"
#ifdef D3D10
#undef D3D10
#endif
//---------------------------------------------------------------------------
...
void BlurGaussian8VSV(float4x4 Proj,
    float3 Input,
    float2 TexCoord,
    float4 Mask,
    float Offset0,
...
Output Main(Input In)
{
    Output Out;
	// Function call #0
    BlurGaussian8VSV(g_Proj, In.Position, In.UVSet, float4(0.0, -1.0, 1.0, 0.0), 
        float(0.5), float(1.5), float(2.5), float(3.5), Out.PosProjected, 
